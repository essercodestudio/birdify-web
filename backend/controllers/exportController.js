...
exports.exportTournamentToExcel = async (req, res) => {
    ...
    try {
        ...
        const tiebreakerRules = [
            { name: "Últimos 9", holes: [10,11,12,13,14,15,16,17,18], frac: 1/2 },
        ...
        const allCategories = ['Absoluto Gross', 'Absoluto Net', ...categories.map(c => c.name)];
        ...
        allCategories.forEach(tabName => {
            ...
            filtered = filtered.filter(p => {
                ...
                if (tabName === 'Absoluto Gross' || tabName === 'Absoluto Net') return true;
                ...
                if (tabName.includes('Sênior') || tabName.includes('Duplas')) return true;
                return false;
            });
            ...
            sheet.columns = [
                { header: 'Pos.', width: 6 }, { header: 'Nome', width: 30 }, { header: 'HDC', width: 8 },
                { header: 'B1', width: 5 }, { header: 'B2', width: 5 }, { header: 'B3', width: 5 },
                { header: 'B4', width: 5 }, { header: 'B5', width: 5 }, { header: 'B6', width: 5 },
                { header: 'B7', width: 5 }, { header: 'B8', width: 5 }, { header: 'B9', width: 5 },
                { header: '1ª Volta', width: 10 },
                { header: 'B10', width: 5 }, { header: 'B11', width: 5 }, { header: 'B12', width: 5 },
                { header: 'B13', width: 5 }, { header: 'B14', width: 5 }, { header: 'B15', width: 5 },
                { header: 'B16', width: 5 }, { header: 'B17', width: 5 }, { header: 'B18', width: 5 },
                { header: '2ª Volta', width: 10 },
                { header: 'GROSS', width: 10 }, { header: 'NET', width: 10 },
            ...
            const parRow = sheet.addRow([
                '', 'PAR DO CAMPO', '',
                parArray[1], parArray[2], parArray[3], parArray[4], parArray[5], parArray[6], parArray[7], parArray[8], parArray[9], parOut,
                parArray[10], parArray[11], parArray[12], parArray[13], parArray[14], parArray[15], parArray[16], parArray[17], parArray[18], parIn,
            ...
            filtered.forEach((p, index) => {
                ...
                sheet.addRow([
                    index + 1, p.name, p.handicap,
                    p.scores[1] || '-', p.scores[2] || '-', p.scores[3] || '-', p.scores[4] || '-', p.scores[5] || '-', p.scores[6] || '-', p.scores[7] || '-', p.scores[8] || '-', p.scores[9] || '-', outSum,
                    p.scores[10] || '-', p.scores[11] || '-', p.scores[12] || '-', p.scores[13] || '-', p.scores[14] || '-', p.scores[15] || '-', p.scores[16] || '-', p.scores[17] || '-', p.scores[18] || '-', inSum,
                    p.gross, p.net, p.tieReason || ''
