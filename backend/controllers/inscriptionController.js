...
exports.getTournamentDetails = async (req, res) => {
  try {
    ...
    if (tourResults.length === 0) {
      return res
        .status(404)
        .json({ message: "Torneio não encontrado ou acesso negado." });
    }
    ...
    const [catResults] = await db.execute(
      "SELECT * FROM tournament_categories WHERE tournament_id = ?",
      [id],
    );
    tournament.categories = catResults;

    // Busca os patrocinadores desse torneio
    const [sponResults] = await db.execute(
      "SELECT * FROM tournament_sponsors WHERE tournament_id = ?",
      [id],
    );
...
exports.createInscription = async (req, res) => {
  try {
    ...
    if (existingInscriptions.length > 0) {
      return res
        .status(400)
        .json({ message: "Já estás inscrito neste torneio!" });
    }
