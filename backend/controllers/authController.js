...
exports.forgotPassword = async (req, res) => {
    try {
        ...
        const [users] = await db.execute(query, [email]);
        if (users.length === 0) {
            return res.status(404).json({ message: 'Se esse e-mail estiver cadastrado, um link será enviado.' }); // Mensagem genérica por segurança
        }
       ...
       // Configura o E-mail usando as variáveis do .env
        ...
        res.status(500).json({ error: 'Erro ao enviar e-mail', message: error.message });
    }
};
...
exports.resetPassword = async (req, res) => {
    try {
        ...
        const query = 'SELECT id FROM users WHERE reset_password_token = ? AND reset_password_expires > NOW()';
        const [users] = await db.execute(query, [token]);

        if (users.length === 0) {
            return res.status(400).json({ message: 'O link é inválido ou já expirou. Solicite novamente.' });
        }
